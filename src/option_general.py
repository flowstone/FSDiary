...
class OptionGeneral(MenuWindow):
    ...
    def create_advanced_group(self):
        ...
        self.breathing_light_checkbox.setChecked(self.config_manager.get_config(FsConstants.APP_MINI_BREATHING_LIGHT_CHECKED_KEY))
    ...
    def save_settings(self):
        ...
        try:
            ...
            self.config_manager.set_config(FsConstants.APP_MINI_BREATHING_LIGHT_CHECKED_KEY, self.breathing_light_checkbox.isChecked())
            self.config_manager.set_config(FsConstants.APP_MINI_CHECKED_KEY, mini_enabled)  # 将 悬浮球修改状态写入到配置文件
            self.config_manager.set_config(FsConstants.APP_TRAY_MENU_CHECKED_KEY, tray_menu_enabled)  # 将 托盘图标修改的状态写入到配置文件
            if mini_enabled:
                self.config_manager.set_config(FsConstants.APP_MINI_SIZE_KEY, self.slider_value)
                self.config_manager.set_config(FsConstants.APP_MINI_IMAGE_KEY, self.float_ball_path_input.text().strip())
            if tray_menu_enabled:
                self.config_manager.set_config(FsConstants.APP_TRAY_MENU_IMAGE_KEY, self.tray_menu_path_input.text().strip())
